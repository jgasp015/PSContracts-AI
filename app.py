...
# ---------------------------
# 2. THE ENGINE (REAL-TIME SEARCH UPDATED)
# ---------------------------
...
def run_ai(text, prompt, is_compliance=False, is_header=False, is_search=False, is_scope=False):
    ...
    # REAL-TIME CLOCK (The Brain of the App)
    ...
    simple_translation_rules = f"""
    STRICT TRANSLATION RULES:
    1. Act as a Plain English Translator.
    2. Today's Date and Time is EXACTLY: {current_time_str}.
    3. If the user asks about deadlines or "is it too late", compare the bid date to {current_time_str}.
    4. Explain legal jargon in simple, 5th-grade terms.
    """

    if is_compliance:
        system_rules = f"RULES: 1. BE DIRECT. 2. Today is {current_time_str}. 3. Extract SLAs."
    elif is_header:
        system_rules = f"RULES: 1. Extract ONLY proper names. 2. Today is {current_time_str}. 3. If deadline passed, say 'CLOSED'."
    elif is_search:
        # UPDATED: Injected the real-time clock directly into the search prompt
        system_rules = f"{simple_translation_rules}\nTASK: Answer the user's question using the document. Use the current date ({current_time_str}) to answer any timing questions."
    elif is_scope:
        system_rules = f"{simple_translation_rules}\nTASK: Explain the whole job in simple terms."
    else:
        system_rules = f"CORE INSTRUCTION: 1. List physical hardware only. 2. Today is {current_time_str}."
...
if st.session_state.get("active_bid_text"):
    ...
    user_q = st.text_input("Enter your query (e.g., 'Is it too late to bid?'):", key="active_q")
    if user_q: 
